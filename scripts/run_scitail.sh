#!/bin/bash
# SciTail: Real scientific NLI benchmark
set -e
...
LR=1e-4
LORA_RANK=8
mkdir -p results/scitail logs

run_one() {
    local label=$1 seed=$2
    local logfile="logs/scitail_${label}_s${seed}.log"
...
device = 'cuda'
train = SciTailCoopDataset('train', max_samples=10000, seed=$seed)
val = SciTailCoopDataset('val', max_samples=2000, seed=$seed)
...
if is_text:
    ...
    max_tok = 16 if 'trunc' in label else None
    class TextModel(nn.Module):
        ...
        def forward(self, contexts, questions, training=False):
            texts = [f'Premise: {\" \".join(c.split()[:max_tok]) if max_tok else c} Hypothesis: {h}'
                     for c, h in zip(contexts, questions)]
...
with open('results/scitail/${label}_s${seed}.json', 'w') as f:
    json.dump({'label': '$label', 'seed': $seed, 'final_val_acc': vc/vt, 'time': time.time()-t0}, f)
" > "$logfile" 2>&1
    echo "  Done: \$(grep FINAL $logfile)"
}

echo "=== SciTail: Real Scientific NLI Benchmark ==="

for seed in 42 123 456; do
    run_one "text_full" $seed
    run_one "text_trunc16" $seed
    run_one "text_trunc16_lora" $seed
...
for f in sorted(Path('results/scitail').glob('*.json')):
    with open(f) as fh:
        r = json.load(fh)
...
print('-'*48)
for name in ['text_full', 'text_trunc16', 'text_trunc16_lora', 'ssr_d32', 'ssr_d32_lora', 'nocomm']:
    if name in by_name:
        accs = by_name[name]
